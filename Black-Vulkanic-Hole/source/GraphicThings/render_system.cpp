...
namespace engine
{
	...
	void RenderSystem::RenderGameObjects(VkCommandBuffer commandBuffer, std::vector<GameObject>& gameObjects)
	{
		...
		{
			obj.transform.rotation.y = glm::mod(obj.transform.rotation.y + .02f, glm::two_pi<float>());
			obj.transform.rotation.x = glm::mod(obj.transform.rotation.x + .01f, glm::two_pi<float>());
